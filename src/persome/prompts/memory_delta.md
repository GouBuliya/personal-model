...
- `assertions`: durable facts about an entity learned this session (state changes, completed outcomes, stated preferences). Each: `{"subject": <ref-or-new_entity object>, "text": "<one dense self-contained past-tense sentence with all explicit who/what/when(absolute date)/where/how-much/why>", "polarity": "<+|-|0>", "facets": ["<2-6 atomic retrieval handles>"], "quote": ..., "confidence": ...}`. `polarity` is `"0"` for a plain established fact, `"-"` for a stance the owner declined or stopped ("considered X but did not choose it", "used to do X, later stopped"), `"+"` for an explicitly affirmed choice or positive valence. A declined/stopped stance must read as a stance, never as a bare fact. `facets` (optional): 2-6 short atomic addressing handles drawn from the fact — proper nouns, dates, quantities, activity phrases — used ONLY to help retrieval find this memory later; they are never shown as facts themselves, so the dense `text` must stand alone without them.
