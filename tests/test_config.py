...
def test_debug_hud_defaults_to_stage_only(tmp_path: Path) -> None:
    cfg = config.load(tmp_path / "missing.toml")
    assert cfg.debug_hud.show == ["stage"]


def test_debug_hud_show_override(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
    path.write_text('[debug_hud]\nshow = ["stage", "tool_call", "health"]\n')
    cfg = config.load(path)
    assert cfg.debug_hud.show == ["stage", "tool_call", "health"]


def test_set_debug_hud_show_replaces_existing_line() -> None:
    text = '[capture]\nx = 1\n\n[debug_hud]\nshow = ["stage"]\n'
    out = config.set_debug_hud_show(text, ["stage", "health"])
    assert 'show = ["stage", "health"]' in out
...
def test_set_debug_hud_show_appends_section_when_absent() -> None:
    text = "[capture]\nx = 1\n"
    out = config.set_debug_hud_show(text, ["stage", "tool_call"])
    assert "[debug_hud]" in out
    assert 'show = ["stage", "tool_call"]' in out
    assert config.load.__module__  # sanity
    import tomllib

    assert tomllib.loads(out)["debug_hud"]["show"] == ["stage", "tool_call"]
